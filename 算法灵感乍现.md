...
![image-20250627140338468](算法灵感乍现.assets/image-20250627140338468.png)
...
![image-20250627140621497|500](算法灵感乍现.assets/image-20250627140621497.png)	(中间是下标)
...
![image-20250627140706581](算法灵感乍现.assets/image-20250627140706581.png)	![image-20250627140716083|236](算法灵感乍现.assets/image-20250627140716083.png)
...
![image-20250627140814317](算法灵感乍现.assets/image-20250627140814317.png)	
...
class Solution {
    public List<List<String>> groupAnagrams(String[] strs) {
        ...
        for(String cur : strs){
            ...
            Arrays.sort(temp);
            String newCur = new String(temp);
            List<String> list = map.getOrDefault(newCur, new ArrayList<String>());
            list.add(cur);
            map.put(newCur, list);
        }
...
![image-20250627140824283|400](算法灵感乍现.assets/image-20250627140824283.png)		

我们随便试几个数，会得出三种可能：1，循环，无限大

第一二种比较好验证，主要是第三种，我们可以试着把每一位为9时，即最大值对应的next求出来：

![image-20250627140828778|475](算法灵感乍现.assets/image-20250627140828778.png)	
...
![image-20250627140838394](算法灵感乍现.assets/image-20250627140838394.png)	
...
![image-20250915221129923](算法灵感乍现.assets/image-20250915221129923.png)

前缀和+暴力双指针

但是这里需要加入一个pre[0]=0的前置0，也就是pre要整体后移
...
class Solution {
    public int minSubArrayLen(int target, int[] nums) {
        ...
        for(int i=0; i<=n; i++){
            int j = n;
            while(i<j && pre[j]-pre[i]>=target){
                minn=Math.min(minn, j-i);
                j--;
            }
        }
        return minn==100001?0:minn;
    }
}
```
...
class Solution {
    public int firstMissingPositive(int[] nums) {
        int n = nums.length;
        for(int i=0; i<n; i++){
            //细节1：while,并且是nums[i]-1，细节是nums[i]在(0, n]，而不是i
...
![image-20250804165728494](算法灵感乍现.assets/image-20250804165728494.png)
...
![image-20250910222743303](算法灵感乍现.assets/image-20250910222743303.png)
...
![image-20250913151904825](算法灵感乍现.assets/image-20250913151904825.png)

```java
class Solution {
    public String addStrings(String num1, String num2) {
        int i = num1.length()-1, j=num2.length()-1, last=0;
        StringBuilder res = new StringBuilder();
...
![image-20250919204935784](算法灵感乍现.assets/image-20250919204935784.png)

i为左边界，j为右边界负责遍历

大题思路：把s的字符标记为0，t的字符标记为-1。用cnt表示当前已有元素的数量，等于t长度开始结算。遍历时给遍历到的字符++，缩边时给遍历到的字符--。

问题1：怎么保证结算时所有t都在里面呢？，把t字符初始化为当前次数-1(可能会遇到aa则为-2)，遇到一次就+1，然后为0，之后遇到就不为-1，则cnt不变；
...
class Solution {
    public String minWindow(String s, String t) {
        ...
        int sLen = s.length(), tLen = t.length(), cnt=0, begin=-1, resLen = Integer.MAX_VALUE;
        //i是左边界，j遍历s
        for(int i=0, j=0; j<sLen; j++){
            //小于0，说明是t中的字符
            if(map.get(ss[j])<0) cnt++;
            map.put(ss[j], map.get(ss[j])+1);
            //当前滑动窗口到达t长度，因为只有是t字符才cnt++，所以一定包含所有t字符
            while(cnt==tLen){
                if(resLen>j-i+1){
                    begin=i;
                    resLen=j-i+1;
                ...
                //开始缩边，
                if(map.get(ss[i])==0) cnt--;
                map.put(ss[i], map.get(ss[i])-1);
                i++;
            }
        }
        return begin==-1?"":s.substring(begin, begin+resLen);
    }
}
```
...
![image-20250923114941075](算法灵感乍现.assets/image-20250923114941075.png)
...
![image-20250924200759860](算法灵感乍现.assets/image-20250924200759860.png)
...
![image-20250925154018105](算法灵感乍现.assets/image-20250925154018105.png)

平方根用二分，小于1让r为1，大于1正常二分

算精度就用nums[mid]*nums[mid]与输入值进行相减，与精度比较



# 寻找两个正序数组的中位数2*

![image-20251011174956228](算法灵感乍现.assets/image-20251011174956228.png)

```java
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        //别忘了half
        int n = nums1.length, m = nums2.length, half = (n+m)/2;
        //就是换个顺序让nums1始终<nums2，这样只用写一种情况的逻辑
        if(n>m) return findMedianSortedArrays(nums2, nums1);
        //ij是操作nums1的
        int i = 0, j = n;
        while(i<=j){
            //别忘了res
            int mid = (i+j)/2, res = half-mid;
            //l1 r1代表nums1的左右半，l2 r2代表nums2的左右半
            //易错点：mid-1和mid而不是+1,不然会越界，以及n与m
            int l1 = (mid==0) ? Integer.MIN_VALUE : nums1[mid-1];
            int r1 = (mid==n) ? Integer.MAX_VALUE : nums1[mid];
            int l2 = (res==0) ? Integer.MIN_VALUE : nums2[res-1];
            int r2 = (res==m) ? Integer.MAX_VALUE : nums2[res];
            //nums1和2都是左<对方右则找到了
            if(l1 <= r2 && l2 <=r1){
                //易错点：n+m总长度，分奇偶
                if((n+m) % 2 != 0) return Math.min(r1, r2);
                //左半取大，右半取小，因为要靠中间
...
class LRUCache {
    ...
    public void put(int key, int value) {
        ListNode nowNode = cache.get(key);
        if(nowNode==null){
            nowNode = new ListNode(key, value);
            //错1，忘记size++
            size++;
            addToHead(nowNode);
    ...
    public ListNode removeTail(){
        //错3，把tailNode当作正常node，用正常remove代码
...
![image-20250819164356642](算法灵感乍现.assets/image-20250819164356642.png)
...
class Solution {
    public void reorderList(ListNode head) {
        if(head==null) return;
        ListNode cur = head;
        ListNode reverseHead = reverse(getHalf(head));
        while(cur!=null){
            ListNode tempL = cur.next;
            ListNode tempR = reverseHead.next;
            cur.next = reverseHead;
            reverseHead.next = tempL;
            cur=tempL;
            reverseHead=tempR;
        }
    }

    public ListNode getHalf(ListNode curNode){
...
**简单一句话，针对当前cur的左右进行递归爆搜，左右都搜到则说明你就是公共祖先**
...
![image-20250824222807149](算法灵感乍现.assets/image-20250824222807149.png)

细节：根节点到叶子节点，可以借用dfs的写法，进入循环前temp.add出来后temp.remove
...
class Solution {
    ...
    public List<List<Integer>> pathSum(TreeNode root, int targetSum) {
        ...
        List<Integer> temp = new ArrayList<>();
        temp.add(root.val);
        traverse(root, targetSum, root.val, temp);
        return res;
    }

    public void traverse(TreeNode curNode, int targetSum, int sum, List<Integer> temp){
        if(curNode.left==null && curNode.right==null && sum==targetSum) {
            res.add(new ArrayList<>(temp));
        }
        if(curNode.left!=null){
            temp.add(curNode.left.val);
            traverse(curNode.left, targetSum, sum+curNode.left.val, temp);
            temp.remove(temp.size()-1);
        }
        if(curNode.right!=null){
            temp.add(curNode.right.val);
            traverse(curNode.right, targetSum, sum+curNode.right.val, temp);
            temp.remove(temp.size()-1);
        }
    }
}
...
class Solution {
    ...
    public void traverse(TreeNode root, int floor){
        if(root==null) return;
        if(res.size()==floor){
            res.add(new ArrayList<>());
        }
...
- List也用的linkedList()，就可以在加入结果时直接new LinkedList<>(nowDeque)
- 临时容器必须是Deque方便加左加右，最初用的ArrayList，就只能从左到右加，就得考虑其他的添加顺序，容易被绕进去
- **就正常中左右遍历，只是在添加到临时容器时对当前node考虑下加左还是右**
...
class Solution {
    public List<List<Integer>> zigzagLevelOrder(TreeNode root) {
        ...
        //常规bfs
        //TIP 这里一定得是队列，先进先出，for的原理就是利用固定数量的先进先出来达到每一层的效果，
...
![image-20250703151458384](算法灵感乍现.assets/image-20250703151458384.png)

![image-20250703151541004](算法灵感乍现.assets/image-20250703151541004.png)

因为中序是左中右嘛，就可以利用"先左"的特性，来查看当前node的左子树是否构造完毕

前序是中左右，因此第一个node就是根，需要构造的node我们就入栈，val等于inorder[inorderIndex]，说明没有左子树/左子树构造完毕。

构造完毕左子树我们就需要构造右子树，因为不一定都有右节点，因此while向上回溯，直到第一个有右节点的数

TIP：

**preorderVal只用来构造，判断还是bfs的机制，通过node**
...
## 从后序与中序构造二叉树7
![](算法灵感乍现.assets/image-20250706180222589.png)

后续是中右左，因此就两种情况：

1. v是u的右儿子
2. v不是u的右儿子，是某个祖宗节点/自己的左儿子，那么下一步便是到v

我们只需要将中序和后续反着读即可与`前序和中序`写起来一样了：
因此inorderIndex要从后往前，for(postorder也要从后往前)
...
class Solution {
    public TreeNode buildTree(int[] inorder, int[] postorder) {
        ...
        for(int i=postorder.length-2; i>=0; i--){
            ...
            TreeNode curNode = nodes.peekLast();
            if(curNode.val!=inorder[inorderIndex]){
                curNode.right=new TreeNode(postorderValue);
                nodes.addLast(curNode.right);
            }else{
                //只有这里才poll!!!!!
                while(!nodes.isEmpty() && nodes.peekLast().val == inorder[inorderIndex]){
                    curNode = nodes.pollLast();
                    inorderIndex--;
                }
                curNode.left=new TreeNode(postorderValue);
...
**先判断target，左右都有对应的特殊情况，然后else都是一样的，大于起点就缩l，小于就缩r**
...
2025.7.20 主要还是关注下为1的情况，并且我们r=length，属于非常规，要注意下
...
class Solution {
    ...
    public boolean culculate(int[] citations, int n){
        int temp=0;
...
- 峰值是严格大于左右的，而**最大值**也是峰值，只需要找到峰值即可
...
class Solution {
    ...
    public int orangesRotting(int[][] grid) {
        ...
        //存储上一轮的坏橘子
        Deque<Position> lastTerm = new LinkedList<Position>(); 
        while(!deque.isEmpty()){
            Position org = deque.pollFirst();
            int ox = org.x;
            int oy = org.y;
            for(int i=0; i<4; i++){
                int x = ox+nextX[i], y=oy+nextY[i];
                if(x<0 || x>=xLen || y<0 || y>=yLen) continue;
                if(grid[x][y]==1) {
                    lastTerm.addLast(new Position(x, y));
                    grid[x][y]=2;
                }
            }
            //如果空了，说明这一轮结束了，把上一轮的放进去
            if(deque.isEmpty() && !lastTerm.isEmpty()){
                res++;
                while(!lastTerm.isEmpty()){
                    deque.addLast(lastTerm.pollFirst());
                }
            }
        }
		//结尾看是否还有新鲜橘子
...
- 入度用数组，依赖用`List<List>`， 入度为0用Deque，结果集用List
- for的结束条件用numCourses还是依赖的length



## 克隆图1

![image-20250620153355779](算法灵感乍现.assets/image-20250620153355779.png)

为了防止死循环，需要用哈希表记录是否有克隆节点，有则直接返回，没有再克隆
...
TIP：new完后就得put，不然递归完再put的话会栈溢出
...
其实就是维护两个栈，只需要在入栈的时候看看是否是当前最小，因为栈是先进后出，如果从顶到底是abcd，此时顶是b的话，cd一定在
...
2025.6.10：k得传入n-k，因为排序结果是递增的；**k值起一个定位的左右，排完肯定是有序的，k的话就是n-k位就是我们最终的目标**，因此具体排序逻辑还是nums[l]而不是nums[k]。

同时包括我们最后的if条件，我们只需要排序k在的部分就行（本身这种排序就是分成一个一个排序区间，不需要的区间就不用排了）
...
思路：用map计数，然后放入优先队列（给优先队列自定义比较逻辑），然后取最终结果
```java
class Solution {
...
![image-20250710151836004](算法灵感乍现.assets/image-20250710151836004.png)

从左往右，先算最大，再更最小，这样可以避免顺序问题
...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        for(int i=0; i<prices.length; i++){
            maxn=Math.max(maxn, prices[i]-minn);
            minn=Math.min(minn, prices[i]);
        }
        return maxn==-1?0:maxn;
    }
}
```
...
其实就是当天卖了后可以重新买，也就是卖了后minn为当前price然后continue；
...
![image-20251003182452671](算法灵感乍现.assets/image-20251003182452671.png)
    ...
    public int longestValidParentheses(String s) {
        ...
        for(int i=1; i<s.length(); i++){
            if(s.charAt(i)==')'){
                if(s.charAt(i)=='('){
                    //易错点：直接赋值而不是和dp[i]比
                    //易错点：判断i>=2，而且是d[i-2]+2
                    dp[i]=((i>=2)?dp[i-2]:0)+2;
                }
                //易错点：i-dp[i-1]-1>=0,这里和下面都是同样的运算前先检查是否会越界
                else if((i-dp[i-1]-1>=0) && s.charAt(i-dp[i-1]-1)=='('){
                    //易错点：要把dp[i-1]带上，是i-dp[i-1]>=2而不是i-dp[i-1]-1>=2
                    //易错点：应该是dp[i-dp[i-1]-2]而不是-1
...
![image-20250611163840614](算法灵感乍现.assets/image-20250611163840614.png)

核心：dp的常规要点：该点由上一个位置的点的最小路径决定，具体看注解

```java
class Solution {
    public int minPathSum(int[][] grid) {
        //第一行的元素由左决定，第一列的元素由上决定
        //不在第一行/列的元素，由其上或者其左元素的最小路径决定，因此可以使用DP[][]
...
![image-20250613151419327](算法灵感乍现.assets/image-20250613151419327.png)

思路：`dp[i][j]`表示能到ij的路径和，由i-1,j和i,j-1决定

优化：滚动数组，dp[j]表示i-1,j，dp[j-1]表示i,j-1

```java
class Solution {
    public int uniquePathsWithObstacles(int[][] obstacleGrid) {
        int n = obstacleGrid.length, m = obstacleGrid[0].length;
        //滚动数组，dp[j-1]表示左一个，dp[j]表示上一个
        int[] dp = new int[m];
...
![image-20250614205957093](算法灵感乍现.assets/image-20250614205957093.png)

![image-20250614210010814](算法灵感乍现.assets/image-20250614210010814.png)

```java
class Solution {
    public boolean isInterleave(String s1, String s2, String s3) {
        //f[i][j]表示s3的前i+j字符能否由s1前i和s2前j交错组成
        //当s1的第i个元素=s3的第i+j个元素，能否组成就取决于s1的前i-1个元素和s3的i+j-1是否相等也就是f[i-1][j]，对于s2的j同理，得出状态转移方程
        //f[i-1][j] && s1.charAt(i-1)==s3.charAt(p)
        //f[i][j-1] && s2.charAt(j-1)==s3.charAt(p)
        //这里的话还得考虑边界，所以写成下面的样子
...
第二次写的细节： `boolean[n+1][m+1]`并且i<=n, j<=m



## 二维：编辑距离 (难)8

![image-20250615210922679](算法灵感乍现.assets/image-20250615210922679.png)

题解：[72. 编辑距离 - 力扣（LeetCode）](https://leetcode.cn/problems/edit-distance/?envType=study-plan-v2&envId=top-interview-150)
...
class Solution {
    public int minDistance(String word1, String word2) {
        ...
        //边界,很好理解,另一个为0时,则需要操作i或者j次才能成为对方
        for(int i=0; i<n+1; i++){
            dp[i][0]=i;
        }
        for(int j=0; j<m+1; j++){
        ...
        //对A插入等价于对B删除、对A替换等价于对B替换
        //因此存在3种情况：A插、B插、修改
        for(int i=1; i<n+1; i++){
            for(int j=1; j<m+1; j++){
                int left = dp[i-1][j]+1;
                int right = dp[i][j-1]+1;
                //修改：1不同则修改(+1)，2本来就相同则不用修改(不+1)
...
![image-20250618154929250](算法灵感乍现.assets/image-20250618154929250.png)
...
![image-20250714171301057](算法灵感乍现.assets/image-20250714171301057.png)

- 如果i-1和j-1对应字符串相等，则`dp[i-1][j-1]+1`
- 否则去i-1或j-1的max
- 从1开始，结束也是<=
...
**核心，用多数去抵消少数，剩下的肯定是多数，通过count+-，candidate为当前多数**
...
其实就是p0是首，p2是尾，i是剩下的第三个颜色

i遇到了0就和p0换位置，遇到了2就和p2换位置，只不过p2要一直while往前，(因为我们是从左往右)
...
**前置**：龟兔赛跑法：假设他俩都在链表上跑，如果有环，兔子肯定先进环，然后一直在环内循环，就必然乌龟会在环内某一点与兔子相遇。
...
数学公式：慢指针从起点触发，快指针从相遇点触发，相同步数相遇后的点必然是入口点。
